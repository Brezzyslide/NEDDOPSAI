...
describe("Worker Profile Registry integrity", () => {
  it("contains exactly 47 worker profiles (including all current v2 specialists through KDS)", () => {
    expect(WORKER_PROFILES).toHaveLength(47);
  });
...
describe("Role-to-Worker-Profile mapping", () => {
  ...
  it("ROLE_TO_PROFILES covers all 47 mapped specialist identities", () => {
    expect(Object.keys(ROLE_TO_PROFILES)).toHaveLength(47);
  });
