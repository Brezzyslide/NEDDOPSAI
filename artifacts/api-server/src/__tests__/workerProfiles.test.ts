...
describe("Worker Profile Registry integrity", () => {
  it("contains exactly 43 worker profiles (including current v2 APO, BSI, PGS, SDC, WRC, WCS, Payroll, P&C, T&L and PAC)", () => {
    expect(WORKER_PROFILES).toHaveLength(43);
  });
...
describe("Role-to-Worker-Profile mapping", () => {
  ...
  it("ROLE_TO_PROFILES covers all 43 mapped specialist identities", () => {
    expect(Object.keys(ROLE_TO_PROFILES)).toHaveLength(43);
  });
