...
describe("Canonical Workforce DNA Foundation", () => {
  ...
  it("has no remaining current-v2 DNA-pending specialists after KDS activation", () => {
    ...
    expect(getCanonicalDNAProfile("talent_learning_specialist")).not.toBeNull();
    ...
    expect(getCanonicalDNAProfile("financial_planning_reporting_manager")).not.toBeNull();
    expect(getSafeDNADescriptor("financial_planning_reporting_manager")).not.toBeNull();
  });
  ...
  it("maps Talent & Learning Specialist as the current v2 learning specialist", () => {
    const dna = getCanonicalDNAProfile("talent_learning_specialist");
    expect(dna).not.toBeNull();
    expect(dna?.identity.specialistId).toBe("talent_learning_specialist");
    expect(dna?.professionalMission.missionStatement).toContain("workforce capability");
    expect(dna?.domainExpertise.competencies.length).toBeGreaterThanOrEqual(24);
    expect(dna?.requiredWorkerProfile.profileCode).toBe(TALENT_LEARNING_SPECIALIST_DNA.requiredWorkerProfile.profileCode);
  });

  it("maps Process & Asset Coordinator as the current v2 process and asset specialist", () => {
    const dna = getCanonicalDNAProfile("process_asset_coordinator");
    expect(dna).not.toBeNull();
    expect(dna?.identity.specialistId).toBe("process_asset_coordinator");
  ...
  it("maps Financial Planning & Reporting Manager as the current v2 FP&R specialist", () => {
    const dna = getCanonicalDNAProfile("financial_planning_reporting_manager");
    expect(dna).not.toBeNull();
    expect(dna?.identity.specialistId).toBe("financial_planning_reporting_manager");
