...
import { executionIntentsTable, withTenantContext } from "@workspace/db";
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const intents = await withTenantContext(
        { tenantId: ctx.tenantId, userId: ctx.userId, purpose: "execution_intents.list" },
        (tx) => tx
          .select()
