...
const FUNDING_UTILISATION_SECTIONS = [
  methodGateSection("USER_DEFINITION_REQUIRED_METHOD", "Method Source Status", "Approved funding-utilisation review method, plan-management source or human-defined review framework.", 10, ["service_agreement"]),
  section("FUNDING_SCOPE_AND_SOURCES", "Funding Scope and Sources", "Participant plan/funding context, service agreement, delivered supports, period and source status.", "Use verified participant funding/service records and state gaps.", 20, ["service_agreement"]),
  section("UTILISATION_VARIANCE_AND_GAPS", "Utilisation, Variance and Gaps", "Utilisation evidence, variance, under/over-use risks and missing records.", "Do not make legal, plan-management or claiming determinations.", 30),
  section("ACTIONS_HANDOFFS_AND_APPROVAL", "Actions, Handoffs and Approval", "SDC/finance actions, participant communication needs, approvals and unresolved decisions.", "Route finance, service agreement and participant commitment decisions to the right owner.", 40),
];
...
export const BLUEPRINT_REGISTRY: RegistryEntry[] = [
  ...
  },
  {
    ...
    professionalAuthority: "mixed",
    futureOwnerRoleCode: "service_delivery_coordinator",
    supportingSpecialists: ["finance_officer", "financial_planning_reporting_manager", "knowledge_documentation_specialist"],
    deliverableContract: structuredAnalysisDeliverable("funding_utilisation_review", ["utilisation_variance_review"], ["claiming_decision", "legal_determination", "service_agreement_change"]),
    evidenceContract: participantEvidence(["service_agreement", "actual_service_delivery"], ["funding_record", "participant_context", "financial_record"], 2, "block_completion"),
    sections: FUNDING_UTILISATION_SECTIONS,
    requiredLibraryKnowledge: ["service_agreement", "pricing_policy", "participant_plan_context"],
    requiredEntityKnowledge: { participant: true },
    requiredApprovals: { human_professional_method_owner: true, service_delivery_owner: true, finance_owner: true },
    validationRules: [{ rule: "funding_utilisation_method_source_or_user_definition_required", required: true, description: "Funding utilisation review must cite approved method/source or keep USER_DEFINITION_REQUIRED visible." }],
    successCriteria: ["Funding and service records are cited", "Utilisation variance is evidence-backed", "Claiming/service-agreement decisions remain approval-gated"],
    outputTypes: ["funding_utilisation_review"],
    escalationRules: [{ trigger: "claiming_or_service_agreement_change_required", action: "require_finance_sdc_and_human_approval" }],
    mandatoryCitations: ["service_agreement", "actual_service_delivery"],
    externalAuthorityRequiredFor: ["claiming decision", "plan-management/legal determination", "service agreement change"],
  },
