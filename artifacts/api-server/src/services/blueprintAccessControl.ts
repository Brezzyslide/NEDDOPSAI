...
/**
 * Filter a blueprint version snapshot response through the same platform-IP
 * boundary as live blueprint detail responses.
 */
export function filterBlueprintVersionForRole(
  version: Record<string, unknown>,
  ctx: BlueprintAccessContext,
): Record<string, unknown> {
  if (ctx.isPlatformAdmin) return { ...version };
  ...
  const filteredSnapshot = filterBlueprintForRole(snapshot as Record<string, unknown>, ctx);
  return {
    ...version,
...
export function filterBlueprintVersionsForRole(
  versions: Record<string, unknown>[],
  ctx: BlueprintAccessContext,
): Record<string, unknown>[] {
  return versions.map(version => filterBlueprintVersionForRole(version, ctx));
}
